...
module AWS
 class SimpleDB
  ...
  def query_with_attributes(domain_name, query_expression=nil, attribute_name=nil, options={:fetch_all=>true})
    more_items = true
    next_token = nil
	...
	'AttributeName' => attribute_name,
      ...
      xml_doc.elements.each('//Item') do |item_node|
        item_node.elements.each('Name') do |name_node|
          name_node.elements.each('//Attribute') do |attr_node|
            attr_name = attr_node.elements['Name'].text
            value = attr_node.elements['Value'].text
